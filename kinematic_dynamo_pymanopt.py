...
def cost(vec1,vec2):
    ...
    vecToField(vec1/np.sqrt(factor), u)
    vecToField(vec2/np.sqrt(factor), B_init)

    # print('int u',d3.integ(u@u).evaluate()['g']/(2*np.pi)**3)
    # print('int B',d3.integ(B_init@B_init).evaluate()['g']/(2*np.pi)**3)
...
def grad(vec1,vec2):
    ...
    u_update.change_scales(1)
    solver.sens_adj[0].change_scales(1)
    u_update['g'] = solver.sens_adj[0]['g']
    solver_u.solve()
    ...
    return [gradU/np.sqrt(factor),gradA/np.sqrt(factor)]
...
dUx0 /= np.linalg.norm(dUx0)
Ux0 /= np.linalg.norm(Ux0)
optimizer = ConjugateGradient(verbosity=verbosity)
